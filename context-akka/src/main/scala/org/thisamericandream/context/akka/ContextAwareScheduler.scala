...
class ContextAwareScheduler(config: Config, logAdapter: LoggingAdapter, threadFactory: ThreadFactory) extends LightArrayRevolverScheduler(config, logAdapter, threadFactory) {
  override def schedule(initialDelay: FiniteDuration, delay: FiniteDuration, runnable: Runnable)(implicit executor: ExecutionContext): Cancellable = {
    val ctx = Context.get()
